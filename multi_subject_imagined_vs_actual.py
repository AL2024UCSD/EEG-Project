...
class SubjectDataExtractor(ImaginedVsActualAnalyzer):
    ...
    def _extract_lateralization_features(self) -> Dict:
        ...
        if left_real and right_real and 'C3' in left_real[0].ch_names and 'C4' in left_real[0].ch_names:
            ...
            # Store mean ERPs
            features['left_fist']['real']['c3'] = left_real_all.get_data()[:, c3_idx, :].mean(axis=0)
            features['left_fist']['real']['c4'] = left_real_all.get_data()[:, c4_idx, :].mean(axis=0)
            features['right_fist']['real']['c3'] = right_real_all.get_data()[:, c3_idx, :].mean(axis=0)
            features['right_fist']['real']['c4'] = right_real_all.get_data()[:, c4_idx, :].mean(axis=0)
            
            features['left_fist']['imagined']['c3'] = left_imag_all.get_data()[:, c3_idx, :].mean(axis=0)
            features['left_fist']['imagined']['c4'] = left_imag_all.get_data()[:, c4_idx, :].mean(axis=0)
            features['right_fist']['imagined']['c3'] = right_imag_all.get_data()[:, c3_idx, :].mean(axis=0)
            features['right_fist']['imagined']['c4'] = right_imag_all.get_data()[:, c4_idx, :].mean(axis=0)
            
            # Calculate lateralization indices
            li_left_real = (features['left_fist']['real']['c3'] - features['left_fist']['real']['c4']) / \
                          (features['left_fist']['real']['c3'] + features['left_fist']['real']['c4'] + 1e-10)
            li_right_real = (features['right_fist']['real']['c3'] - features['right_fist']['real']['c4']) / \
                           (features['right_fist']['real']['c3'] + features['right_fist']['real']['c4'] + 1e-10)
            
            li_left_imag = (features['left_fist']['imagined']['c3'] - features['left_fist']['imagined']['c4']) / \
                          (features['left_fist']['imagined']['c3'] + features['left_fist']['imagined']['c4'] + 1e-10)
            li_right_imag = (features['right_fist']['imagined']['c3'] - features['right_fist']['imagined']['c4']) / \
                           (features['right_fist']['imagined']['c3'] + features['right_fist']['imagined']['c4'] + 1e-10)
            
            features['lateralization_index']['real']['left'] = li_left_real
            features['lateralization_index']['real']['right'] = li_right_real
            features['lateralization_index']['imagined']['left'] = li_left_imag
            features['lateralization_index']['imagined']['right'] = li_right_imag
    ...
    def aggregate_data(self):
        ...
        for subject_id, data in self.subject_data.items():
            ...
            if 'lateralization' in data:
                ...
                if lat['lateralization_index']['real']['left'] is not None:
                    self.aggregated_data['lateralization']['real']['left'].append(
                        lat['lateralization_index']['real']['left'])
                if lat['lateralization_index']['real']['right'] is not None:
                    self.aggregated_data['lateralization']['real']['right'].append(
                        lat['lateralization_index']['real']['right'])
                if lat['lateralization_index']['imagined']['left'] is not None:
                    self.aggregated_data['lateralization']['imagined']['left'].append(
                        lat['lateralization_index']['imagined']['left'])
